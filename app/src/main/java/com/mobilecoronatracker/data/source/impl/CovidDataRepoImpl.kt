...
import com.mobilecoronatracker.data.networking.impl.safeApiCall
...
class CovidDataRepoImpl(private val covidApi: CovidApi) : CovidDataRepo {
    override suspend fun getCountriesData(): List<CountryReportModelable> {
        return when (val safeApiCallResult = safeApiCall { covidApi.getCountriesData() }) {
    ...
    override suspend fun getCumulatedData(): GeneralReportModelable {
        return when (val safeApiCallResult = safeApiCall { covidApi.getCumulatedData() }) {
