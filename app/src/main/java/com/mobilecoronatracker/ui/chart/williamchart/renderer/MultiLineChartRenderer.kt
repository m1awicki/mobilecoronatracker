...
class MultiLineChartRenderer(
...
) : ChartContract.MultiLineRenderer {
    ...
    override fun render(entries: List<LinkedHashMap<String, Float>>) {
        data = entries.map {
            it.toDataPoints()
        }
        view.postInvalidate()
    ...
    override fun anim(labels: List<String>, entries: List<LinkedHashMap<String, Float>>, animation: ChartAnimation<DataPoint>) {
        data = entries.map {
            it.toDataPoints()
        }
        xLabels = labels.map {
